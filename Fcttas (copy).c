...
void siftDownv2(T_heap *p, int k) {
...
void addElt(T_heap *p, T_elt e) {
	p->tree[p->nbElt] = e; 
	p->nbElt++; 
	siftDownv2(p,p->nbElt-1);
}


void buildHeapv3(T_heap *p){
	int k;
	for(k=1;k<p->nbElt; k++) {
		siftDownv2(p, k);
	}	
...
void heapSortv3(T_heap *p){
	...
	while(p->nbElt < 1){
		d-> tree[k] = removeMin(p);
		d->nbElt ++;
		d-> tree[k+1] = removeMin(p);
		d->nbElt ++;
		j = d->tree[k] + d->tree[k+1];
		addElt(p, j);
		k++;
	}
